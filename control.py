...
def WorkCycle(mode, grill_platform, adc_device, display_device, dist_device):
    ...
    # Set time since last fan speed update
    FanUpdateTime = starttime
    ...
    while status == 'Active':
        ...
        if control['hopper_check'] or (now - hoppertoggletime > 300):
            pelletdb = ReadPelletDB()
        ...
        # If in "Hold" mode, recompute Fan speed
        if (mode == 'Hold'):
            if now - FanUpdateTime > 6:
                if CycleRatio > 0.35:
                    newdc = 95
                elif CycleRatio > 0.3:
                    newdc = 85
        ...
        if (current_output_status['auger'] == AUGEROFF) and (now - augertoggletime > CycleTime * (1 - CycleRatio)):
            ...
            if mode == 'Hold':
                ...
                if settings['globals']['debug_mode']:
                    event = '* On Time = ' + str(OnTime) + ', OffTime = ' + str(OffTime) + ', CycleTime = ' + str(
                        CycleTime) + ', CycleRatio = ' + str(CycleRatio) + ', PID = ' + str(PIDControl.getPID())
                    print(event)
                    WriteLog(event)
            if settings['globals']['debug_mode']:
